...
- [x] **Phase 6: One-Shot Disk↔FTP Sync Through Library API** - Developers can run one-shot local→FTP and FTP→local sync through `FTPSyncService` while preserving v1 FTP behavior and cwd safety. (completed 2026-04-27)
- [x] **Phase 7: Background Disk→FTP Lifecycle** - Developers can start, observe, and stop persistent local disk→FTP sync with deterministic cleanup and no FTP→disk polling API. (completed 2026-04-27)
- [x] **Phase 8: FTP-Only Package Reduction** - The library build exposes only FTP sync capabilities and drops or isolates unrelated CLI/server/protocol dependencies. (completed 2026-04-28)
- [x] **Phase 9: Verification, Examples, and Migration Docs** - Tests and docs prove the library contract, real FTP flows, limitations, and migration path. (completed 2026-04-28)
...
- Public API centers on `FTPSyncService` with typed Go options only.
- One-shot sync returns compact `Result + error` data and preserves best-effort partial-failure behavior.
- Background sync is intentionally local disk→FTP only in v2.0.
- Package-local FTP and retry helpers replaced legacy runtime adapters before old runtime package trees were removed.
- The final supported local-library import path is `ftpsync/ftpsync`.

**Issues Resolved:**

- One-shot disk↔FTP sync is available through the library API without CLI invocation.
- FTP→local writes are constrained to the configured local endpoint and covered by cwd-safety regression tests.
- Background local disk→FTP sync has deterministic stop/wait behavior and non-terminal error observation.
- Old CLI/server/protocol/runtime dependencies are excluded from the reduced package graph.
- README, examples, migration notes, and real FTP integration tests match the final package contract.

**Issues Deferred:**

- FTPS/TLS support.
- FTP→local background polling.
- FTP↔FTP sync and bidirectional conflict resolution.
- Legacy YAML/CLI parser support in the public API.
- FTP server mode.

**Technical Debt Accepted at Close:**

- Pre-close audit status was `tech_debt` due to planning source-of-truth drift, not implementation gaps.
- Drift affected ROADMAP, REQUIREMENTS, PROJECT, and STATE metadata after Phase 6-9 completion.
- The drift was accepted by the user at close and reconciled in the milestone archive commit.
